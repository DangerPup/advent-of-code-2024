...
def isRecordSafe(lLevels: list) -> bool:
    ...
    #   2. Any adjacent levels are different by 1 to 3 inclusive, allow for at most one level to break this rule.
...
while (i<retList.__len__()):
    ...
    if(isRecordSafe(lLevels)==True):
        ...
        # Problem Dampener, if not safe, try to find a safe version with one level dropped out.
        k=0
        while(k<lLevels.__len__()):
            l=0
            dampenLevels=[]
            while(l<lLevels.__len__()):
                if(k!=l):
                    dampenLevels.append(lLevels[l])
                l+=1
            if(isRecordSafe(dampenLevels)==True):
                countOfSafeRecords+=1
                break
            k+=1
