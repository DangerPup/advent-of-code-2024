...
# Returns a new map with most recent guard location and guard in new location with possible new heading.
# Runs until the guard leaves the map.
def RecurseThroughMap(map: list, currPos:list, currHeading: str) -> list:
    ...
    # Recurse if the guard is still on the map.   
    if(leftTheMapSw):
        return map
    else:
        return RecurseThroughMap(map, [nextX, nextY] , nextHeading)
...
while(i<lMap.__len__()):
    if(headings['north']  in lMap[i]):
        lCurrGuardPosition=[i,lMap[i].index("^")]
        sCurrGuarHeading = "^"
        break
    ...
    if(headings['south']  in lMap[i]):
        lCurrGuardPosition=[i,lMap[i].index("v")]
        sCurrGuarHeading = "v"
        break
...
#PrintMap(lMap)
RecurseThroughMap(lMap, lCurrGuardPosition, sCurrGuarHeading)
